...
Pageviews are counted by [GoatCounter](https://www.goatcounter.com). The
dashboard is at **https://nklnklnklnklnklnkl.goatcounter.com**.

No cookies are set and no personal data is stored, so no consent banner is
needed.

- **Excluding your own visits:** load `https://nkl.online#toggle-goatcounter`
  once in a browser and that browser stops being counted. Repeat per browser
  or device. You can also add your IP under *Settings → Tracking → Ignore IPs*.
- **Local development is already ignored** — requests from localhost and
  private networks are skipped by default.
- **Counts run low.** Adblockers block GoatCounter, so real traffic is
  somewhat higher than reported. Useful for trends, not exact figures.

The `<link rel="canonical">` in `index.html` matters here: without it the apex
and `www` versions of the domain are counted as two separate pages.
